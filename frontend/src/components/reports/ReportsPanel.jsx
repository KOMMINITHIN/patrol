...
const ReportsPanel = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      ref={panelRef}
      className="reports-panel-container flex flex-col"
    >
      {/* Header */}
      <div className="p-4 pb-0">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">{stats.total} community reports</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tabs */}
        <div className="flex bg-gray-100/80 rounded-xl p-1.5 mb-4">
          <button
          ...
          >
            <span className={`flex items-center justify-center gap-2 ${
              activeTab === 'create' ? '' : 'text-blue-600'
            }`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {activeTab === 'feed' ? (
          <div className="px-4 pb-4">
            ...
            <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2 -mx-1 px-1">
              <div className="flex items-center gap-2 px-3.5 py-2 bg-blue-50 rounded-xl text-sm whitespace-nowrap border border-blue-100">
                <span className="w-2.5 h-2.5 bg-blue-500 rounded-full"></span>
                <span className="text-blue-700 font-semibold">{stats.open} Open</span>
              </div>
              <div className="flex items-center gap-2 px-3.5 py-2 bg-amber-50 rounded-xl text-sm whitespace-nowrap border border-amber-100">
                <span className="w-2.5 h-2.5 bg-amber-500 rounded-full"></span>
                <span className="text-amber-700 font-semibold">{stats.inProgress} In Progress</span>
              </div>
              {stats.urgent > 0 && (
                <div className="flex items-center gap-2 px-3.5 py-2 bg-red-50 rounded-xl text-sm whitespace-nowrap border border-red-100 animate-pulse">
                  <span className="w-2.5 h-2.5 bg-red-500 rounded-full"></span>
                  <span className="text-red-700 font-semibold">{stats.urgent} Urgent</span>
                </div>
              )}
            </div>
            ...
            {/* Filter panel */}
            {showFilters && (
              <div className="space-y-4 mb-4 p-4 bg-gray-50/80 rounded-xl border border-gray-100">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Status</label>
                  <select
                    value={filters.status}
                    onChange={(e) => handleFilterChange('status', e.target.value)}
                    className="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all"
                  >
                ...
                <button
                  ...
                  className="w-full py-2.5 text-sm text-primary-600 hover:text-primary-700 hover:bg-primary-50 rounded-xl font-semibold transition-colors"
                >
                  Clear All Filters
                </button>
              </div>
              ...
              <div className="space-y-3">
                {reports.map((report) => (
                  <div
                    ...
                    className="report-card-mini cursor-pointer group"
                  >
                    <div className="flex gap-3.5">
                      {/* Thumbnail */}
                      <div className="w-[88px] h-[88px] rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 shadow-sm">
                        {report.photo_url ? (
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center text-2xl bg-gray-50">📷</div>';
                            }}
                          />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0 py-0.5">
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          ...
                          <span className={`badge text-xs flex-shrink-0 ${getStatusBadge(report.status)}`}>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs text-gray-500 font-medium">{getCategoryLabel(report.category)}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          ...
                          <span className="font-medium">{formatTimeAgo(report.created_at)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
